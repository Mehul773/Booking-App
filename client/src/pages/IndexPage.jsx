import axios from "axios";
import { useEffect, useState } from "react";
import * as myConst from "../MyConstant";
import {Link} from "react-router-dom"

export default function IndexPage() {
  const [places, setPlaces] = useState([]);
  useEffect(() => {
    axios.get("/all-places").then((res) => {
      setPlaces(res.data);
    });
  }, []);
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-4 gap-2 mx-auto">
      {places.length > 0 &&
        places.map((place) => (
          <>
            <Link to={"/place/"+place._id} className=" p-3 flex flex-col gap-1 cursor-pointer">
              <div className="h-60 overflow-hidden rounded-lg">
                
                <img
                  src={myConst.BACKEND_URL + "uploads/" + place.photos[0]}
                  alt="img"
                  srcset=""
                  className="rounded-xl aspect-square object-cover max-h-80  hover:scale-110 transition-all duration-500"
                />
              </div>
              <h2 className="font-thin truncate">{place.title}</h2>
              <h3 className="text-base">{place.address}</h3>
              <p className="flex gap-1">
                <span className="text-primary">{place.price}</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={1.5}
                  stroke="currentColor"
                  className="w-6 h-6"
                  >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z"
                    />
                </svg>
                per night
              </p>
            </Link>
          </>
        ))}
    </div>
  );
}
